...
# Add new session state variables for UI control
...
for repo_name in repo_names:
    if repo_name not in st.session_state.repo_url_mapping:
        ...
        if os.path.exists(f"./repositories/{repo_name}/.git"):
            ...
            st.session_state.repo_url_mapping[repo_name] = f"local/{repo_name}"
if 'last_submitted_selection' not in st.session_state:
    st.session_state.last_submitted_selection = []
with st.form("my_form"):
    ...
    submitChatRepo = st.form_submit_button("Chat with selected repositories")

# Handle form submission and repository selection changes
if submitChatRepo:
    if selected_repo:
        if selected_repo != st.session_state.current_selection:
            st.session_state.show_chat = False
        st.session_state.current_selection = selected_repo
        
        # Only show chat if submitChatRepo was clicked
        if submitChatRepo:
